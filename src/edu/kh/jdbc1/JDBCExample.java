...
public class JDBCExample {
	public static void main(String[] args) {
		...
		// * JDBC를 이용한 이플리케이션을 망들 때 필요한 것
		// 1. Java의 JDBC 관련 인터페이스
		// 2. DBMS(Oracle)
		// 3. Oracle에서 Java 애플리케이션과 연결할 때 사용 할
		...
		// * DB 와 Java 사이를 연결해주는 통로(Stream과 유사함)
		...
		try {
			...
			// oracle.jdbc.driver.T4CConnection@971d0d8  -> 드라이버 매니저가 커넥션 생성을 했다는 뜻
			
			
			// 3. SQL 작성
			// ** JAVA에서 작성되는 SQL은 마지막에 ;(세미콜론) 찍지 않기! **
			String sql = "SELECT EMP_ID, EMP_NAME, SALARY, HIRE_DATE FORM EMPLOYEE";
			...
			// 3단계 : SQL을 수행해서 받은 결과(Resulet Set)를
			//			한행씩 접근해서 컬럼 값 얻어오기
			...
			System.out.printf("사번 : %s / 이름 : %s / 급여 : %d / 입사일 : %s\n",
				...
				// java.sql.Date.dml toString()은 yyyy-mm-dd 형식으로 오버라이딩 됭어있음
			...
			System.out.println("JDBC 드라이브 경로가 잘못 작성되었습니다.");
		} catch(SQLException e){
			// SQLExeption : DB 관련 최상위 예외 ( == 데이터베이스에 관련된 모든 예외)
		}finally {
			// 4단계 : 사용한 JDBC 객체 자원 반환(close())
			// ResultSet, statement, connection 단기 (생성 역순으로 닫는 것을 권장)
			try {
				if(rs != null) rs.close();
